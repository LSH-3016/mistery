    ...
    switch (scenario.case_title) {
      case '베이지 갤러리의 정전':
        return (
          <>
            ...
            {/* 컵 */}
            <Cylinder args={[0.08, 0.06, 0.15]} position={[-1, 0.85, -4.5]} castShadow receiveShadow>
              <meshStandardMaterial color="#ffffff" roughness={0.4} />
            </Cylinder>

            {/* 펜 */}
            <Cylinder args={[0.015, 0.015, 0.2]} position={[0.5, 0.76, -4.8]} rotation={[0, 0, Math.PI / 6]} castShadow>
              <meshStandardMaterial color="#1a1a1a" roughness={0.6} />
            </Cylinder>

            {/* 노트 */}
            <Box args={[0.25, 0.02, 0.35]} position={[1.2, 0.76, -4.5]} castShadow receiveShadow>
      ...
      case '심야 연구실의 비밀':
        return (
          <>
            {/* 비커들 */}
            <Cylinder args={[0.1, 0.12, 0.25]} position={[-2, 0.88, -4.5]} castShadow receiveShadow>
              <meshStandardMaterial color="#88ccff" transparent opacity={0.6} roughness={0.1} />
            </Cylinder>
            <Cylinder args={[0.08, 0.1, 0.2]} position={[-1.5, 0.85, -4.6]} castShadow receiveShadow>
              <meshStandardMaterial color="#ff8888" transparent opacity={0.6} roughness={0.1} />
            </Cylinder>

            {/* 시약병들 */}
            <Cylinder args={[0.06, 0.08, 0.2]} position={[1.5, 0.85, -4.8]} castShadow receiveShadow>
              <meshStandardMaterial color="#66ff66" transparent opacity={0.7} />
            </Cylinder>
            <Cylinder args={[0.06, 0.08, 0.2]} position={[1.7, 0.85, -4.8]} castShadow receiveShadow>
              <meshStandardMaterial color="#ffff66" transparent opacity={0.7} />
            </Cylinder>
            <Cylinder args={[0.06, 0.08, 0.2]} position={[1.9, 0.85, -4.8]} castShadow receiveShadow>
              <meshStandardMaterial color="#ff66ff" transparent opacity={0.7} />
            </Cylinder>

            {/* 현미경 */}
            <Cylinder args={[0.08, 0.12, 0.3]} position={[0, 0.9, -4.5]} castShadow receiveShadow>
              <meshStandardMaterial color="#2a2a2a" metalness={0.8} roughness={0.3} />
            </Cylinder>
            <Cylinder args={[0.04, 0.04, 0.25]} position={[0, 1.2, -4.5]} rotation={[Math.PI / 6, 0, 0]} castShadow>
              <meshStandardMaterial color="#1a1a1a" metalness={0.9} roughness={0.2} />
            </Cylinder>

            {/* 노트북 */}
            <Box args={[0.4, 0.02, 0.3]} position={[-3.5, 0.76, -4.5]} castShadow receiveShadow>
              <meshStandardMaterial color="#1a1a1a" metalness={0.6} roughness={0.4} />
            </Box>
            <Box args={[0.4, 0.3, 0.02]} position={[-3.5, 0.91, -4.65]} rotation={[-Math.PI / 12, 0, 0]} castShadow>
              <meshStandardMaterial color="#0a0a0a" />
            </Box>
            <Box args={[0.35, 0.25, 0.01]} position={[-3.5, 0.91, -4.64]} rotation={[-Math.PI / 12, 0, 0]}>
            ...
            {/* 시험관 거치대 */}
            <Box args={[0.5, 0.05, 0.15]} position={[4, 0.78, -5]} castShadow receiveShadow>
              <meshStandardMaterial color="#8b7355" roughness={0.7} />
            </Box>
            <Cylinder args={[0.02, 0.02, 0.15]} position={[3.8, 0.88, -5]} castShadow>
              <meshStandardMaterial color="#88ccff" transparent opacity={0.6} />
            </Cylinder>
            <Cylinder args={[0.02, 0.02, 0.15]} position={[4, 0.88, -5]} castShadow>
              <meshStandardMaterial color="#ff8888" transparent opacity={0.6} />
            </Cylinder>
            <Cylinder args={[0.02, 0.02, 0.15]} position={[4.2, 0.88, -5]} castShadow>
      ...
      case '고층 빌딩의 추락':
        return (
          <>
            ...
            {/* 커피잔 */}
            <Cylinder args={[0.08, 0.06, 0.12]} position={[-0.8, 0.81, -4.5]} castShadow receiveShadow>
              <meshStandardMaterial color="#ffffff" roughness={0.4} />
            </Cylinder>
            <Cylinder args={[0.075, 0.075, 0.1]} position={[-0.8, 0.81, -4.5]}>
              <meshStandardMaterial color="#3d2817" roughness={0.8} />
            </Cylinder>

            {/* 펜꽂이 */}
            <Cylinder args={[0.08, 0.1, 0.15]} position={[1, 0.83, -4.8]} castShadow receiveShadow>
              <meshStandardMaterial color="#2a2a2a" roughness={0.6} />
            </Cylinder>
            <Cylinder args={[0.015, 0.015, 0.2]} position={[0.95, 0.98, -4.8]} castShadow>
              <meshStandardMaterial color="#1a1a1a" />
            </Cylinder>
            <Cylinder args={[0.015, 0.015, 0.18]} position={[1.05, 0.97, -4.8]} castShadow>
              <meshStandardMaterial color="#0066cc" />
            </Cylinder>

            {/* 모니터 */}
            <Box args={[0.6, 0.4, 0.05]} position={[-3.5, 1, -4.5]} castShadow receiveShadow>
              <meshStandardMaterial color="#1a1a1a" metalness={0.7} roughness={0.3} />
            </Box>
            <Box args={[0.55, 0.35, 0.02]} position={[-3.5, 1, -4.48]}>
              <meshStandardMaterial color="#0a0a0a" />
            </Box>
            <Box args={[0.5, 0.3, 0.01]} position={[-3.5, 1, -4.47]}>
              <meshStandardMaterial color="#1a3a5a" emissive="#0a2a4a" emissiveIntensity={0.2} />
            </Box>

            {/* 키보드 */}
            <Box args={[0.45, 0.02, 0.15]} position={[-3.5, 0.76, -4]} castShadow receiveShadow>
              <meshStandardMaterial color="#2a2a2a" roughness={0.5} />
            </Box>

            {/* 마우스 */}
            <Box args={[0.08, 0.04, 0.12]} position={[-3, 0.77, -4]} castShadow receiveShadow>
